...
export function IncludedSection() {
  return <section id="included" className="bg-[#111] text-white py-24 px-4 md:px-6 border-t border-white/5">
      <div className="max-w-7xl mx-auto">
        <motion.div className="flex items-center gap-4 mb-16" initial={{
        opacity: 0,
        x: -20
      }} whileInView={{
        opacity: 1,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => <GlassCard key={index} className="p-8 min-h-[280px] flex flex-col justify-between group" hoverEffect initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
